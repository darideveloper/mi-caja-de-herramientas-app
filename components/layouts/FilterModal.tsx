...
import { View, Modal, Pressable, Image, TouchableOpacity, ActivityIndicator, PanResponder, Animated, Dimensions, ScrollView } from 'react-native';
...
interface FilterModalProps {
  isVisible: boolean;
  onClose: () => void;
  onApplyFilters?: (filters: { groupId?: number; categoryIds: number[]; duration?: number }) => void;
}
  ...
  return (
    ...
    >
      <View className={`flex-1 justify-end bg-black/50`}>
        <View className={`bg-white rounded-t-3xl p-8`} style={{ maxHeight: '90%' }}>
          <ScrollView contentContainerStyle={{ paddingBottom: 24 }} showsVerticalScrollIndicator={false}>
            ...
            <View className="mb-6">
              <Text className="mb-2 text-base text-black font-semibold">Estados de ánimo</Text>
              <Dropdown
                options={groups}
            ...
            {/* Categories from API */}
            <View className="mb-6">
              <Text className="mb-2 text-base text-black font-semibold">Categorías</Text>
              {categoriesLoading ? (
                ...
                <View className="flex-row flex-wrap gap-2">
                  {categories.map((cat) => (
                    <TouchableOpacity
                      ...
                      className={`flex-row items-center px-4 py-2 rounded-full border ${selectedCategories.includes(cat.id) ? 'border-purple bg-purple/10' : 'border-purple/30 bg-white'}`}
                      style={{ marginBottom: 8 }}
                    >
                      <Image source={{ uri: cat.icon }} style={{ width: 22, height: 22, marginRight: 8, borderRadius: 11 }} />
                      <Text className={`text-base ${selectedCategories.includes(cat.id) ? 'text-purple' : 'text-black'}`}>{cat.name}</Text>
                    </TouchableOpacity>
                  ))}
                </View>
              )}
          ...
          <View className={`flex-row justify-between gap-4`} style={{ marginTop: 16 }}>
            <Btn
              onPress={onClose}
              className={`flex-1 !bg-transparent border-2 border-purple`}
            >
              <Text className={`text-purple text-lg`}>Cancelar</Text>
            </Btn>
            <Btn
              onPress={handleApplyFilters}
              className={`flex-1`}
            >
              <Text className={`text-green text-lg`}>Aplicar</Text>
            </Btn>
          </View>
        </View>
      </View>
